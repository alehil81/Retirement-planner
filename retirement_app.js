...
const GENERIC={
 ...
 high3:200000,divYield:1.3,expenses:150000,ordinaryTaxPct:20,dividendTaxPct:15,fixedPct:3.5,userClaim:70,spouseClaim:70,mode:"need",startDate:"2018-12-01"
};
const LIMITS={currentAge:[18,90],spouseAge:[18,100],retirementAge:[19,100],targetPortfolio:[0,1e11],inflationPct:[0,20],realReturnPct:[-10,20],vooBalance:[0,1e11],k401Balance:[0,1e11],voo1:[0,1e9],k1:[0,1e9],vooIncreasePct:[-100,50],kIncreasePct:[-100,50],changeAge:[18,100],voo2:[0,1e9],k2:[0,1e9],high3:[0,1e7],divYield:[0,15],expenses:[0,1e8],ordinaryTaxPct:[0,60],dividendTaxPct:[0,40],fixedPct:[0,20]};
...
const commas=n=>new Intl.NumberFormat("en-US",{maximumFractionDigits:2}).format(n);
...
function showWarning(msg){$("warning").textContent=msg;$("warning").classList.add("show");setTimeout(()=>$("warning").classList.remove("show"),4000)}
...
document.querySelectorAll("[data-key]").forEach(el=>{
 ...
 el.addEventListener("blur",()=>{const k=el.dataset.key,n=parseNum(el.value),prev=Number(el.dataset.prev);if(n===null||!valid(k,n)){s[k]=Number.isFinite(prev)?prev:GENERIC[k];showWarning("That value is outside the allowed range, so the previous value was restored.")}else{s[k]=n;savePlan()}showInput(el);render()});
...
function taxesFor(div,fers,ss401,wd){
 const ord=s.ordinaryTaxPct/100,divTax=s.dividendTaxPct/100;
 return div*divTax+fers*ord+ss401*.85*ord+wd*ord;
}
function retirement(p,f){
 const r=s.realReturnPct/100,start=Math.ceil(s.retirementAge),spouseStart=s.currentAge+(s.spouseClaim-s.spouseAge);
 let voo=p.v,k=p.k;const rows=[];
 for(let age=start;age<=95;age++){
  const spa=s.spouseAge+(age-s.currentAge);
  const u=age>=s.userClaim?ssAnnual(s.userClaim):0,sp=spa>=s.spouseClaim?ssAnnual(s.spouseClaim):0,ss=u+sp;
  const div=voo*s.divYield/100;
  const taxesBase=taxesFor(div,f.annual,ss,0),afterTaxBase=div+f.annual+ss-taxesBase;
  ...
  const taxes=taxesFor(div,f.annual,ss,wd),gross=div+f.annual+ss+wd,afterTax=gross-taxes,surplus=afterTax-s.expenses;
  const endV=Math.max(0,voo*(1+r)-div),endK=Math.max(0,availableK-wd);
  rows.push({age,spa,vooStart:voo,kStart:k,div,fers:f.annual,u,sp,ss,wd,gross,taxes,afterTax,expenses:s.expenses,surplus,baseAfterTax:afterTaxBase,endV,endK,endTotal:endV+endK,required});
  voo=endV;k=endK;
 }
 return{rows,spouseStart};
}
function calc(){
 ...
 const bothAge=Math.max(Math.ceil(s.retirementAge),Math.ceil(s.userClaim),Math.ceil(ret.spouseStart));
 const both=ret.rows.find(x=>x.age>=bothAge)||ret.rows.at(-1);
 return{p,f,ret,target:target?target.age:null,both};
}
...
function render(){
 ...
 $("age95Out").textContent=money(age95.endTotal);$("age95Sub").textContent="VOO "+money(age95.endV)+" · 401(k) "+money(age95.endK);$("targetOut").textContent=c.target?"Age "+Math.round(c.target):"Not reached";
 ...
 $("userSS").querySelectorAll("button").forEach(x=>x.classList.toggle("active",+x.dataset.age===s.userClaim));$("spouseSS").querySelectorAll("button").forEach(x=>x.classList.toggle("active",+x.dataset.age===s.spouseClaim));
 $("userBenefit").textContent=money(ssAnnual(s.userClaim)/12)+"/mo · "+money(ssAnnual(s.userClaim))+"/yr";$("spouseBenefit").textContent=money(ssAnnual(s.spouseClaim)/12)+"/mo · "+money(ssAnnual(s.spouseClaim))+"/yr";
 ...
 $("pre401").textContent=money(b.baseAfterTax)+"/yr";$("preDetail").textContent="After-tax VOO dividends + FERS + Social Security, before 401(k)";
...
function line(h,d,a,key,color,width=2.5){return h+`<polyline points="${d.map(x=>a.x(x.age)+","+a.y(x[key])).join(" ")}" fill="none" stroke="var(${color})" stroke-width="${width}" stroke-linecap="round" stroke-linejoin="round"/>`}
function xTicks(h,d,a){for(let i=0;i<=6;i++){const ag=d[0].age+(d.at(-1).age-d[0].age)*i/6;h+=`<text x="${a.x(ag)}" y="${a.H-14}" text-anchor="middle" font-size="11" fill="var(--muted)">${Math.round(ag)}</text>`}return h}
function draw(existing){
 const c=existing||calc();if(!c)return;
 if(chartMode==="accumulation"){
  const d=c.p.rows,max=Math.max(s.targetPortfolio,...d.map(x=>x.t),1)*1.08,a=axes(max,d[0].age,d.at(-1).age);let h=xTicks(a.g,d,a);
  h=line(h,d,a,"t","--s2",4);h=line(h,d,a,"v","--s3");h=line(h,d,a,"k","--s4");
  const ty=a.y(s.targetPortfolio);h+=`<line x1="${a.L}" y1="${ty}" x2="${a.W-a.R}" y2="${ty}" stroke="var(--text)" stroke-dasharray="6 5"/>`;
  $("chart").innerHTML=h;$("chartSub").textContent="Accumulation through retirement in today’s dollars.";
 }else if(chartMode==="decumulation"){
  const d=c.ret.rows.map(x=>({age:x.age,t:x.endTotal,v:x.endV,k:x.endK})),max=Math.max(...d.map(x=>x.t),1)*1.08,a=axes(max,d[0].age,d.at(-1).age);let h=xTicks(a.g,d,a);
  h=line(h,d,a,"t","--s2",4);h=line(h,d,a,"v","--s3");h=line(h,d,a,"k","--s4");
  $("chart").innerHTML=h;$("chartSub").textContent="Year-by-year real portfolio balances after retirement through age 95.";
 }else if(chartMode==="income"){
  const d=c.ret.rows,max=Math.max(s.expenses,...d.map(x=>x.afterTax),1)*1.1,a=axes(max,d[0].age,d.at(-1).age);let h=xTicks(a.g,d,a);
  const bw=Math.max(5,a.pw/d.length*.62);
  d.forEach(x=>{const xx=a.x(x.age)-bw/2;let base=0;const netDiv=x.div*(1-s.dividendTaxPct/100),netF=x.fers*(1-s.ordinaryTaxPct/100),netSS=x.ss*(1-.85*s.ordinaryTaxPct/100),netWd=x.wd*(1-s.ordinaryTaxPct/100);[[netDiv,"--s2"],[netF,"--s3"],[netSS,"--s5"],[netWd,"--s6"]].forEach(q=>{const y1=a.y(base+q[0]),y0=a.y(base);h+=`<rect x="${xx}" y="${y1}" width="${bw}" height="${Math.max(0,y0-y1)}" fill="var(${q[1]})"/>`;base+=q[0]})});
  const ey=a.y(s.expenses);h+=`<line x1="${a.L}" y1="${ey}" x2="${a.W-a.R}" y2="${ey}" stroke="var(--text)" stroke-width="2" stroke-dasharray="7 5"/>`;
  $("chart").innerHTML=h;$("chartSub").textContent="Estimated after-tax income sources versus after-tax expenses.";
 }else{
